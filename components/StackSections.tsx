...
useEffect(() => {
  const ctx = gsap.context(() => {
    ...
    { scale: 1.5 },
    {
      ...
      scrollTrigger: {
        trigger: panel,
        start: 'top bottom',  // start scaling as it enters
        end: 'top top',        // finish scaling when top hits top
        scrub: true,
      },
    }
  );

      // Animate content (text, items)
      gsap.from([title, paragraph, ...items], {
        opacity: 0,
        y: 30,
        stagger: 0.1,
        duration: 0.6,
      ...
      // Pin and scale section on scroll
    //   gsap.fromTo(panel,
    //     { scale: 0.8},
    //     {
    //       scale: 1,
...
>
  ...
  >
    <h2 className="text-4xl md:text-5xl font-light mb-2">{section.title}</h2>
